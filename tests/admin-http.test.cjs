...
test('administrator HTTP login, isolated reset, logout and server stop require authority', async () => {
  ...
  try {
    ...
    assert.equal(eye.sessionTag,oldState.sessionTag+':0');
    ...
    assert.deepEqual(Object.keys(home.medical.children).sort(),['cortex_echo.app','doctor_note.txt','recovery_service.txt']);
    ...
    await fetch(base+'/api/game/action',{method:'POST',headers:{Cookie:cookie,'Content-Type':'application/json'},body:JSON.stringify({action:'start'})});
    assert.notEqual((await (await fetch(base+'/api/eye?display=1')).json()).sessionTag,tagBeforeRefresh);
